...
import 'package:projeto_dev_disp_mob/widget/loading.dart';
...
class _TrailDetailsPageState extends State<TrailDetailsPage> {
  ...
  void initState() {
    super.initState();
    trail = widget.trail;
    context.read<TrailController>().getTrail(widget.trail.id!).then(
      (ntrail) {
        trail = ntrail ?? trail;
      },
    );
              ...
              Column(
                children: [
                  Container(
                    ...
                    child: FlutterMap(
                      options:
                          MapOptions(initialCenter: trail!.averageLatLng()),
                      children: [
                        TileLayer(
                          urlTemplate:
  ...
  void editTrail(Trail otrail) async {
